...
export const MockpitInput: React.FC<MockpitInputProps> = ({
  ...
  return (
    <div
      className={`relative flex items-center transition-all rounded-xl ${
        isActive ? 'ring-2 ring-sky-500/80 shadow-[0_0_12px_rgba(56,189,248,0.3)]' : ''
      } ${wrapperClassName}`}
    >
      {icon && (
        <div className="absolute left-2.5 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none flex items-center justify-center">
          {icon}
        </div>
      )}
      <input
        ...
        className={`w-full bg-slate-950 border border-slate-800 rounded-xl py-1.5 text-xs text-slate-100 placeholder-slate-500 focus:outline-none focus:border-sky-500/80 font-mono transition-colors ${
